  ...
  try {
    for (let i = 1; i < sortedBackups.length; i++) {
      ...
      try {
        ...
        let totalSrcRanges = 0;
        if (hasTable(srcDb, 'BlockRange')) {
          const srcRangesRes = srcDb.exec('SELECT * FROM BlockRange');
          if (srcRangesRes.length > 0) {
            totalSrcRanges = srcRangesRes[0].values.length;
        ...
        addLog(`  -> Unified ${totalSrcRanges} block range(s) (+${blockRangesAddedThisFile} new/healed)`);
            ...
            for (const row of srcNotesRes[0].values) {
              ...
              if (findNote.step()) {
                ...
                if (lastModified && (!destLastModified || lastModified > destLastModified)) {
                  ...
                  if (destNoteCols.has('Title')) { updateSets.push('Title = ?'); updateVals.push(title); }
                  if (destNoteCols.has('Content')) { updateSets.push('Content = ?'); updateVals.push(content); }
                  if (destNoteCols.has('LastModified')) { updateSets.push('LastModified = ?'); updateVals.push(lastModified); }
                  if (destNoteCols.has('Created')) { updateSets.push('Created = COALESCE(?, Created)'); updateVals.push(created); }
                  if (destNoteCols.has('LocationId')) { updateSets.push('LocationId = ?'); updateVals.push(mappedLocId); }
                  if (destNoteCols.has('UserMarkId')) { updateSets.push('UserMarkId = ?'); updateVals.push(mappedMarkId); }
                  if (destNoteCols.has('BlockType')) { updateSets.push('BlockType = ?'); updateVals.push(blockType); }
                  if (destNoteCols.has('BlockIdentifier')) { updateSets.push('BlockIdentifier = ?'); updateVals.push(blockIdentifier); }

                  if (updateSets.length > 0) {
                    updateVals.push(destNoteId);
                    destDb.run(`UPDATE Note SET ${updateSets.join(', ')} WHERE NoteId = ?`, updateVals);
                  }
                ...
                if (destNoteCols.has('Guid')) { insertCols.push('Guid'); insertVals.push(guid); }
                if (destNoteCols.has('UserMarkId')) { insertCols.push('UserMarkId'); insertVals.push(mappedMarkId); }
                if (destNoteCols.has('LocationId')) { insertCols.push('LocationId'); insertVals.push(mappedLocId); }
                if (destNoteCols.has('Title')) { insertCols.push('Title'); insertVals.push(title); }
                if (destNoteCols.has('Content')) { insertCols.push('Content'); insertVals.push(content); }
                if (destNoteCols.has('Created')) { insertCols.push('Created'); insertVals.push(created || lastModified); }
                if (destNoteCols.has('LastModified')) { insertCols.push('LastModified'); insertVals.push(lastModified); }
                if (destNoteCols.has('BlockType')) { insertCols.push('BlockType'); insertVals.push(blockType); }
                if (destNoteCols.has('BlockIdentifier')) { insertCols.push('BlockIdentifier'); insertVals.push(blockIdentifier); }

                const placeholders = insertCols.map(() => '?').join(', ');
                destDb.run(`INSERT INTO Note (${insertCols.join(', ')}) VALUES (${placeholders})`, insertVals);
        ...
        // Fix for Issue #3: Resolves UNIQUE constraint failed on (TagId, Position)
        const destTagMapCols = getTableColumns(destDb, 'TagMap');
        if (destTagMapCols.size > 0 && hasTable(srcDb, 'TagMap')) {
          const srcTagMapRes = srcDb.exec('SELECT * FROM TagMap ORDER BY Position ASC');
          if (srcTagMapRes.length > 0) {
            ...
            for (const row of srcTagMapRes[0].values) {
              ...
              if (mappedNoteId !== null && mappedNoteId !== undefined) {
                targetType = 1;
                targetTypeId = mappedNoteId;
              } else if (mappedLocId !== null && mappedLocId !== undefined) {
                targetType = 0;
              ...
              // Check if mapping already exists in destDb
              ...
              // Compute next position for mappedTagId to strictly avoid (TagId, Position) unique constraint collisions
              let nextPosition = 0;
              const posStmt = destDb.prepare('SELECT COALESCE(MAX(Position), -1) + 1 FROM TagMap WHERE TagId = ?');
              posStmt.bind([mappedTagId]);
              if (posStmt.step()) {
                nextPosition = posStmt.get()[0] as number;
              }
              posStmt.free();

              if (isDestSchema5) {
                destDb.run('INSERT INTO TagMap (Type, TypeId, TagId, Position) VALUES (?, ?, ?, ?)', [
                  targetType, targetTypeId, mappedTagId, nextPosition
                ]);
                tagMapsAdded++;
              } else {
                destDb.run(`
                  ...
                  VALUES (?, ?, ?, ?, ?)
                `, [mappedPlId, mappedLocId, mappedNoteId, mappedTagId, nextPosition]);
                tagMapsAdded++;
              }
            }
            addLog(`  -> Mapped ${tagMapsAdded} new tag link(s) to TagMap`);
          }
        }

        // 7. Merge Bookmarks
        if (hasTable(destDb, 'Bookmark') && hasTable(srcDb, 'Bookmark')) {
          ...
          if (srcBkRes.length > 0) {
            ...
            for (const row of srcBkRes[0].values) {
              ...
              const findBk = destDb.prepare(`
                SELECT BookmarkId FROM Bookmark 
                WHERE Slot = ? 
                  AND (PublicationLocationId IS ? OR (PublicationLocationId IS NULL AND ? IS NULL))
                LIMIT 1
              `);
              findBk.bind([slot, mappedPubLocId, mappedPubLocId]);

              if (!findBk.step()) {
                destDb.run(`
                  INSERT OR IGNORE INTO Bookmark (LocationId, PublicationLocationId, Slot, Title, Snippet, BlockType, BlockIdentifier)
                  VALUES (?, ?, ?, ?, ?, ?, ?)
        ...
        if (hasTable(destDb, 'InputField') && hasTable(srcDb, 'InputField')) {
          try {
        ...
        if (hasTable(destDb, 'IndependentMedia') && hasTable(srcDb, 'IndependentMedia')) {
          try {
        ...
        if (hasTable(destDb, 'PlaylistItem') && hasTable(srcDb, 'PlaylistItem')) {
          try {
    ...
    try {
      if (hasTable(destDb, 'LastModified')) {
    ...
    stats.totalNotes = (hasTable(destDb, 'Note') && (destDb.exec('SELECT count(*) FROM Note')[0]?.values[0][0] as number)) || 0;
    stats.totalMarks = (hasTable(destDb, 'UserMark') && (destDb.exec('SELECT count(*) FROM UserMark')[0]?.values[0][0] as number)) || 0;
    stats.totalTags = (hasTable(destDb, 'Tag') && (destDb.exec('SELECT count(*) FROM Tag')[0]?.values[0][0] as number)) || 0;
    stats.totalBookmarks = (hasTable(destDb, 'Bookmark') && (destDb.exec('SELECT count(*) FROM Bookmark')[0]?.values[0][0] as number)) || 0;
    try {
      stats.totalInputFields = (hasTable(destDb, 'InputField') && (destDb.exec('SELECT count(*) FROM InputField')[0]?.values[0][0] as number)) || 0;
    } catch (e) {
